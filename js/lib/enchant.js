...
enchant.Label = enchant.Class.create(enchant.Entity, {
	 ...
	 * Added correct word wrapping in multiline Label.
	 * Previous version tried to fit as many character as possible.
	 * Current version tries to fit as many word as possible.
	 * Lines changed :
	 * 
	 */
    cvsRender: function(ctx) {
                ...
                while (text.length > c + amount || ctx.measureText(text.slice(c, c + amount)).width > labelWidth) {
                    ...
                    while (increase > 0) {
                        if (ctx.measureText(buf).width < labelWidth) {
							...
							//if(buf.charAt(0) == " ") buf = buf.slice(1, buf.length);
							buf = buf.slice(0, Math.min(buf.length, buf.lastIndexOf(" "))); //slice the buffer to the last word 
							length = buf.length;
                        } else {
                            length -= increase;
                            buf = text.slice(c, c + length);
							//if(buf.charAt(0) === ' ') buf = buf.slice(1, buf.length);
							buf = buf.slice(0, Math.min(buf.length, buf.lastIndexOf(" ")));
							length = buf.length;
                        }
